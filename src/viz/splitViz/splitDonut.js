...
const update = (data, filteredData) => {
  const counts = coalesceHistogram(histogram(data, filteredData, document.getElementById('splitDonutSelect').value, x => x));
...
const init = (data, filteredData, height, width) => {
  document.getElementById('splitDonutChart').innerHTML = '';
  radius = Math.min(width, height) / 2;

  svg = d3
    .select('#splitDonutChart')
    .append('svg')
    .attr('height', height)
    .attr('width', width)
    .append('g');
